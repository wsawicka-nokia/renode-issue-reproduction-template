...
{
    public class STM32F4_RTC_Fixed : IDoubleWordPeripheral, IKnownSize
    {
        public STM32F4_RTC_Fixed(IMachine machine, ulong wakeupTimerFrequency = DefaultWakeupTimerFrequency)
        {
            ...
            var registerMap = new Dictionary<long, DoubleWordRegister>
            {
                {(long)Registers.TimeRegister, new DoubleWordRegister(this)
                    ...
                    .WithWriteCallback((_, __) => {
                        ...
                        mainTimer.TimeState = mainTimer.TimeState.With(second: (int)(newSU.Value + newST.Value*10), minute: (int)(newmU.Value + newmT.Value*10), hour: (int)(newHU.Value + newHT.Value*10));
                    })
                },
                {(long)Registers.DateRegister, new DoubleWordRegister(this, 0x2101)
                    .WithValueField(0, 4, out newDU, name: "DU",
                        valueProviderCallback: _ => mainTimer.Read(DateTimeSelect.Day, Rank.Units))
                    .WithValueField(4, 2, out newDT, name: "DT",
                    ...
                    .WithValueField(12, 1, out newMT, name: "MT",
                    ...
                    .WithValueField(16, 4, out newYU, name: "YU",
                        valueProviderCallback: _ => mainTimer.Read(DateTimeSelect.Year, Rank.Units))
                    .WithValueField(20, 4, out newYT, name: "YT",
                    ...
                    .WithWriteCallback((_, __) => {
                        ...
                        mainTimer.TimeState = mainTimer.TimeState.With(year: (int)(newYU.Value + newYT.Value*10), month: (int)(newMU.Value + newMT.Value*10), day: (int)(newDU.Value + newDT.Value*10));
                    })
                },
        ...
        private readonly IValueRegisterField newDU;
        private readonly IValueRegisterField newDT;
        private readonly IValueRegisterField newMU;
        private readonly IValueRegisterField newMT;
        private readonly IValueRegisterField newYU;
        private readonly IValueRegisterField newYT;
        private readonly IValueRegisterField newSU;
        private readonly IValueRegisterField newST;
        private readonly IValueRegisterField newmU;
        private readonly IValueRegisterField newmT;
        private readonly IValueRegisterField newHU;
        private readonly IValueRegisterField newHT;
        ...
        {
            public TimerConfig(STM32F4_RTC_Fixed parent)
            {
                this.parent = parent;
            }
            ...
            private readonly STM32F4_RTC_Fixed parent;
        }

        private class AlarmConfig
        {
            public AlarmConfig(STM32F4_RTC_Fixed parent, TimerConfig masterTimer)
            {
                this.parent = parent;
